...
static mp_obj_t mp_disp_drv_framebuffer(mp_obj_t n_obj) {
	...
	return mp_obj_new_bytearray_by_ref(sizeof(lv_color_t) * 480 * 800 , (void *)fb[n]);
}

static void mp_disp_drv_flush(lv_disp_drv_t *disp_drv, const lv_area_t *area, lv_color_t *color_p) {
    display_buffer(area->x1,area->y1,area->x2-area->x1,area->y2-area->y1,(uint16_t*)color_p);
    /*IMPORTANT!!!
...
static bool mp_ts_read(struct _lv_indev_drv_t *indev_drv, lv_indev_data_t *data) {
...
STATIC MP_DEFINE_CONST_FUN_OBJ_1(mp_disp_drv_framebuffer_obj, mp_disp_drv_framebuffer);
